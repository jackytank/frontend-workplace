...
import { ChatEvent, ChatEventHandler, ChatEventType, ChatMessage, IChatService, IStorage, MessageContentType, MessageDirection, MessageEvent, SendMessageServiceParams, SendTypingServiceParams, UpdateState, UserTypingEvent } from "@chatscope/use-chat";
...
export class MyChatService implements IChatService {
    storage?: IStorage;
        ...
        window.addEventListener("chat-protocol", (evt: Event) => {
            ...
            if (type === "message") {
                ...
                if (this.eventHandlers.onMessage && detail.sender !== this) {
                    ...
                    const chatEvent: ChatEvent<ChatEventType.Message> = new MessageEvent({
                        message,
                        conversationId
                    });
                    this.eventHandlers.onMessage(chatEvent);
                }
            } else if (type === "typing") {
