...
public class MyBatisSqlAnalyzer {
    ...
    private String resolveSqlContent(Element statement, Map<String, String> sqlFragments) {
        ...
        // Process all text and includes
        Elements children = statement.children();
        fullSql.append(statement.ownText()).append(" ");
        
        for (Element child : children) {
            if (child.tagName().equalsIgnoreCase("include")) {
                String refId = child.attr("refid");
                String fragmentContent = sqlFragments.get(refId);
                if (fragmentContent != null) {
                    fullSql.append(fragmentContent).append(" ");
                }
            } else {
                fullSql.append(child.text()).append(" ");
            }
        }
    ...
    private boolean isTableActuallyRead(String sql, String tableName, Map<String, String> aliasToTable) {
        ...
        // Look for column references from this table
    ...
    private void addOperation(Map<String, String> tableOperations, String tableName, String operation) {
        ...
        if (ops.contains('C')) result.append('C');
        if (ops.contains('R')) result.append('R');
        if (ops.contains('U')) result.append('U');
        if (ops.contains('D')) result.append('D');
